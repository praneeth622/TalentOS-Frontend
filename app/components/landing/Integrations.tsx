...
export function Integrations() {
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{ rotate }}
        className="absolute top-20 left-1/4 w-64 h-64 rounded-full bg-gradient-to-r from-blue-100/40 to-purple-100/40 blur-3xl"
      />
      <motion.div
        style={{ rotate: useTransform(scrollYProgress, [0, 1], [0, -360]) }}
        className="absolute bottom-20 right-1/4 w-80 h-80 rounded-full bg-gradient-to-r from-indigo-100/40 to-pink-100/40 blur-3xl"
      />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <AnimatedSection className="text-center mb-16">
          ...
          <p className="text-slate-500 max-w-xl mx-auto text-lg">
            ...
            management to Web3 wallets. Connect once, work seamlessly.
          </p>
        </AnimatedSection>
        ...
        >
          {integrations.map((integration, i) => (
            <StaggerItem key={integration.name}>
              <HoverCard
                className="group bg-slate-50 hover:bg-white rounded-2xl p-6 flex flex-col items-center gap-3 border border-slate-100 hover:border-slate-200 transition-all duration-300 cursor-pointer relative overflow-hidden"
                intensity={10}
              >
                ...
                <motion.div
                  className={`${integration.color} transition-all relative z-10`}
                ...
                <div className="relative z-10 text-center">
                  ...
                  <motion.span
                    className="text-xs text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                    initial={{ y: 5 }}
                    whileHover={{ y: 0 }}
                  >
        ...
        >
          <span className="text-sm text-slate-500">Also connects with:</span>
          {["Notion", "Asana", "Trello", "Linear", "Monday"].map((tool, i) => (
            <motion.span
              key={tool}
              className="text-xs font-medium text-slate-600 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-100"
